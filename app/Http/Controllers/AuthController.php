...
class AuthController extends Controller
{
    ...
    public function login(LoginRequest $request)
    {
        ...
        // Generate JWT token for the API guard
        ...
        $cookie = Cookie::make(
            'jwt',                   // Nom du cookie
            $token,                  // Valeur du cookie (token JWT)
            config('sanctum.expiration',60),       // Durée de vie en minutes
            '/',                     // Path
            '.test',                 // <- domaine partagé (authservice.test, user-microservice.test)
            false,                   // Secure (mets true si HTTPS)
            true,                    // HttpOnly
            false,                   // Raw
            'lax'                    // SameSite (peut être "None" si tu veux cross-domain total)
        );
    ...
    {
        \Log::info('Validate token called', [
            'user' => $request->user() ? $request->user()->id : 'null',
            'token' => $request->bearerToken() ? 'exists' : 'missing'
        ]);

        // Utiliser $request->user() au lieu de Auth::guard('sanctum')
        if ($request->user()) {
            $user = $request->user();
            return response()->json([
